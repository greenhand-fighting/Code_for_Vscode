...
class Vehicle:
    def __init__(self):
        self.l_f = 2.7 *0.8 # 前距离
        self.l_r = 2.5 *0.8 # 后距离
        self.w = 2.0  # 车宽
...
class CCRRT:
    ...
    def __init__(self, car, start, goal, obstacle_list, rand_area,obstacle_list_from_pu,obstacle_list_for_pengzhuang_jiance): # 车型，起点，终点，
        ...
        self.obstacle_list = obstacle_list  # 人工椭圆
        self.obstacle_list_from_pu = obstacle_list_from_pu  # 感知椭圆
        self.obstacle_list_for_pengzhuang_jiance = obstacle_list_for_pengzhuang_jiance  # 感知框
    ...
    def get_chance_constrain_from_pu(self, current):
        ...
        delta_t = 0  # sum(min delte_tj)
        # cal for each obs
        for obs in self.obstacle_list_from_pu:
...
def obstacle_uncertainty_fusion(gts, uncertainties):
    ...
    for gt, un in zip(gts, uncertainties):
        ...
        # obs.append((gt[0], gt[1], d_a + un[0], d_b + un[1], gt[4]))
    return obs
def obstacle_uncertainty_fusion_from_pu(gts):
    obs_from_pu = []
    for gt in gts:
        obs_from_pu.append((gt[0], gt[1], gt[5], gt[6], gt[4], gt[5]-gt[2]/2.0, gt[6]-gt[3]/2.0))  #注意，这里的列表中表示的椭圆，使用长短半轴表示的，并不是整个轴！！！
    return obs_from_pu
...
def main():
    ...
    ganzhi_kuang = [
        (1.83, 74.57, 3.82/2, 1.59/2, np.deg2rad(90.0)),
        # (5.36, 44.96, 3.96, 1.59, np.deg2rad(100.0)),
        (5.44, 65.03, 3.56/2, 1.54/2, np.deg2rad(89.0)),
        (5.41, 94.42, 4.28/2, 1.65/2, np.deg2rad(90.0)),
        # (9.0, 54.93, 4.08, 1.59, np.deg2rad(90.0)),
        (9.124, 84.98, 3.93/2, 1.66/2, np.deg2rad(90.0)),
    ...
    zhenzhi_kuang = [
        (1.86, 61.3, 4.51/2, 2.0/2, np.deg2rad(90.0)),

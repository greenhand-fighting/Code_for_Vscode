...
class CCRRT:
    ...
    def __init__(self, car, start, goal, obstacle_list, obstacle_list1,rand_area):
        ...
        self.obstacle_list = obstacle_list   # 感知框，用于碰撞检测的
        self.obstacle_list1 = obstacle_list1   # 感知椭圆，用于计算机会约束的
...
def main():
    ...
    obstacle_list1 = obstacle_uncertainty_fusion1(obstacle_list_gt)   # 这个是计算最后path的cc使用的
    ...
    # x=draw_legend.DrawLegend()
    # x.drawlegend_function()
    #画图例的-------------------------
    ...
    # 第一个图例
    first_legend = plt.legend([AnyObject(), AnyObject1(), AnyObject2(), AnyObject3(), c],
                              ['ego_car(trajectory)', "Starting position", "Prediction bounding box", "ground truth",
                               "Spatial uncertainty"],
    ...
    plt.colorbar()
    plt.axis("equal")
    plt.axis([area[0]-5, area[1]+5, area[2]-15, area[3]])
    plt.legend(loc='upper right')
    plt.grid(True)
    plt.show()
